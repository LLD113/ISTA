...
class Main:
    ...
    def __init__(self):
        ...
        self.ui.textButton.clicked.connect(self.get_text_SaveSrc)  # 保存位置
    ...
    def startCov(self):
        ...
        self.ui.logtextBrowser.append("成功转换模型...")
    ...
    def openFile(self):
        ...
        if item.parent() is not None:
            ...
            subprocess.call(["open", self.project_path + "/" + item.text(0)])
    ...
    def load_fashion_mnist_data(self):
        ...
        x_train = x_train.reshape(x_train.shape[0], *img_shape).astype('float32') / 255
        x_test = x_test.reshape(x_test.shape[0], *img_shape).astype('float32') / 255
        # 这里的img_shape表示维度, 其他的表示训练集和测试集, 种类，以及标签
        # print(x_test.shape)
        # print(len(x_test))
        # exit(0)
        # k = round(0.1 * len(x_train))
    ...
    def loadModel(self):
        ...
        self.model.summary()
        self.ui.logtextBrowser.append("Successfully loaded model: " + self.model_path)

        self.ui.logtextBrowser.append("*" * 50)
        trainableLayers = get_trainable_layers(self.model)
        str1 = "Trainable layers: \n"
        for l in trainableLayers:
            weights = self.model.layers[int(l)].get_weights()
            str1 += "The" + str(l + 1) + "Layer, shape information for weights: " + str(weights[0].shape) + "\n"
    ...
    def getData2(self):
        ...
        self.ui.logtextBrowser.append("Successfully imported dataset: " + self.test_path)
        self.ui.logtextBrowser.append("*" * 50)
    ...
    def dataLoad(self):
        ...
        if self.ui.self_defining.isChecked() == True:
            ...
            self.ui.logtextBrowser.append("Successful selection of minist dataset......")
            self.ui.logtextBrowser.append("*" * 50)
            self.ui.dataset_lineEdit.setText("minist")

        elif self.ui.Ciafr_radioButton.isChecked() == True:
            self.ui.logtextBrowser.append("Successful selection of cifar-10 dataset......")
            self.ui.logtextBrowser.append("*" * 50)
            self.ui.dataset_lineEdit.setText("cifar-10")

        elif self.ui.FashionMinist_radioButton.isChecked() == True:
            self.ui.logtextBrowser.append("Successful selection of fashion-minist dataset......")
            self.ui.logtextBrowser.append("*" * 50)
            self.ui.dataset_lineEdit.setText("fashion-minist")

    # Import text data set confirmation button
    def load_text_data(self):
        self.ui.logtextBrowser.append("Successful selection of dataset......")
        self.ui.logtextBrowser.append("*" * 50)
        self.ui.dataset_lineEdit.setText(self.test_path.split("/")[-1])

    # Gets the label for the imported dataset
    def getLabel(self):
        selected_file = QFileDialog.getOpenFileName(self.ui, "Select dataset label", ".", "CSV(逗号分隔)(*.csv)")
    ...
    def get_text_data(self):
        selected_text_file = QFileDialog.getOpenFileName(self.ui, "Select dataset", ".", "CSV(逗号分隔)(*.csv)")
    ...
    def save_model_new(self):
        ...
        self.ui.logtextBrowser.append("Successfully save the repaired model......")
        self.ui.logtextBrowser.append("*" * 50)
    ...
    def error_repair_1(self):
        ...
        self.ui.logtextBrowser.append("Successful import of suspicious neuron sorting results......")
        self.ui.logtextBrowser.append("*" * 50)
    ...
    def error_repair_2(self):
        ...
        self.ui.logtextBrowser.append("修复前是：" + format(before_accuracy, '.3%'))
        self.ui.logtextBrowser.append("修复后是：" + format(after_accuracy, '.3%'))
        if (after_accuracy * 100 - before_accuracy * 100) > 1:
            self.ui.logtextBrowser.append("修复成功！")
            self.ui.logtextBrowser.append("*" * 50)
        else:
            self.ui.logtextBrowser.append("修复失败，请选择其他修复算子。")
            self.ui.logtextBrowser.append("*" * 50)
        # 显示修复结果
    ...
    def easy_set_all(self):
        try:
            ...
            self.ui.logtextBrowser.append("预加载失败，从头导入模型")
            self.ui.logtextBrowser.append("*" * 50)
        else:
            self.ui.logtextBrowser.append("预加载成功！")
            self.ui.logtextBrowser.append("*" * 50)
    ...
    def errorLocated(self):
        ...
        for layer in trainableLayers:
            ...
            suspicious = suspicious.reshape((number_neuron, 1))  # 可疑度
            layer_index = np.ones((number_neuron, 1)) * int(layer)  # 层
            neuron_index = np.arange(0, len(spectrum)).reshape((number_neuron, 1))  # 神经元
        ...
        #    QTreeWidgetItem(self.root).setText(0, self.error_location)
        QTreeWidgetItem(self.root).setText(0, self.error_location)
        self.ui.logtextBrowser.append("可疑度排序成功......")
        self.ui.logtextBrowser.append("*" * 50)
    ...
    def generate_spectrum(self):
        ...
        self.ui.logtextBrowser.append("频谱信息已保存到" + self.spectrum_path)
        self.ui.logtextBrowser.append("*" * 50)
    ...
    def generateMeta(self):
        self.ui.logtextBrowser.append("正在生成蜕变图像......")
    ...
    def saveCover_setting(self):
        self.ui.logtextBrowser.append("Successful selection of coverage")
        self.ui.logtextBrowser.append("*" * 50)
    ...
    def seedcoverage(self):
        ...
        self.ui.logtextBrowser.append("The current seed dataset coverage are：{}".format(coverage))
        self.ui.logtextBrowser.append("*" * 50)
        coverage_name = ""
    ...
    def generateMin_Max(self):
        ...
        self.ui.logtextBrowser.append("Neuron upper and lower boundaries are generated successfully.....")
        self.ui.logtextBrowser.append("*" * 50)
    ...
    def show_seed_coverage(self):
        ...
        plt.bar(x1, y, bar_width, align="center", color="c", label="seed test dataset", alpha=0.5)
        ...
        plt.xlabel("Type of coverage", fontdict={'size': 16})
        plt.ylabel("Coverage", fontdict={'size': 16})
        plt.title(u'Seed test dataset coverage', fontdict={'size': 16})

        for a, b in zip(index, y):
            plt.text(a, b + 0.01, '%.3f' % b, ha='center', va='bottom', fontsize=9)

        plt.savefig(self.cover_path + "/seed coverage.jpg", dpi=400, bbox_inches='tight')
        plt.show()
    ...
    def getImageCover(self):
        ...
        with open(self.generate_cov_csv, 'a', newline='') as csvfile:
            ...
            writer.writerow(['Serial No.', 'seed test dataset', 'generate test dataset', 'original Label', 'model prediction labels', 'coverage'])
    ...
    def getSaveSrc(self):
        ...
        self.ui.logtextBrowser.append("Successfully select the path where the generated test cases are saved: " + self.generate_save_path)
        self.ui.logtextBrowser.append("*" * 50)
    ...
    def show_figure(self, coverDic, coverage, figure_save_path):
        ...
        plt.bar(x1, y1, bar_width, align="center", color="c", label="seed test dataset", alpha=0.5)
        plt.bar(x1 + bar_width, y2, bar_width, align="center", color="b", label="generate test dataset", alpha=0.5)
        ...
        plt.xlabel("Type of coverage", fontdict={'size': 16})
        plt.ylabel("Coverage", fontdict={'size': 16})
        plt.title(u'Coverage comparison', fontdict={'size': 16})
    ...
    def show_figure2(self, coverDic, coverage, figure_save_path):
        mpl.rcParams['font.sans-serif'] = ['KaiTi', 'SimHei', 'FangSong']  # 汉字字体,优先使用楷体，如果找不到楷体，则使用黑体
        mpl.rcParams['font.size'] = 12  # 字体大小
        mpl.rcParams['axes.unicode_minus'] = False  # 正常显示负号
        ...
        plt.bar(x1, y1, bar_width, align="center", color="c", label="seed test dataset", alpha=0.5)
        plt.bar(x1 + bar_width, y2, bar_width, align="center", color="b", label="optimize test dataset", alpha=0.5)
    ...
    def generate_text_data(self):
        ...
        self.ui.logtextBrowser.append("Successfully generated test cases......")
        self.ui.logtextBrowser.append("*" * 50)
    ...
    def getGenSet(self):
        ...
        self.ui.logtextBrowser.append("In generating test cases......")
        self.ui.logtextBrowser.append("*" * 50)
    ...
    def generate_notext_coverage_compare(self):
        ...
        self.ui.logtextBrowser.append("Successful comparison of test case coverage......")
        self.ui.logtextBrowser.append("*" * 50)
    ...
    def getOptimizePath(self):
        ...
        self.ui.logtextBrowser.append("Successfully select the optimization test case save path: " + self.optimize_path)
        self.ui.logtextBrowser.append("*" * 50)

    # Optimize the picture
    def openOptimize(self):
        model_path = self.model_path  # 模型路径
        seed_path = self.test_path  # 种子图片路径
        seed_label_path = self.label_path  # 种子图片对应的标签路径
        generate_path = self.test_path  # 生成的图片的路径
        genenrate_label_path = self.label_path  # 生成图片对应的标签的路径
        optimize_output_path = self.optimize_path  # 优化后生成的图片的保存路径
        if self.ui.text_radioButton.isChecked() == True:
            generate_text_path = self.generate_save_path + "/generate_text_data.csv"  # 生成的文本的路径
            ...
            self.ui.logtextBrowser.append("Test case optimization in progress......")
            self.ui.logtextBrowser.append("*" * 50)
            if result_flag == True:
                self.ui.logtextBrowser.append("Test case optimization successfully.....")
            else:
                self.ui.logtextBrowser.append("Test case optimization failure.....")
            self.ui.logtextBrowser.append("*" * 50)
        elif self.ui.concolicButton.isChecked() == True:
            concolic_main(model_path, seed_path, seed_label_path, optimize_output_path)  # 调用函数
    ...
    def OptimizeCoverage(self):
        ...
        self.ui.logtextBrowser.append("Successfully calculate the coverage of optimized test cases......")
        self.ui.logtextBrowser.append("*" * 50)
    ...
    def OptimizeCoverageCompare(self):
        ...
        self.show_figure2(self.seed_coverage, self.optimize_coverage, figure_save_path)
        QTreeWidgetItem(self.root).setText(0, figure_save_path)
        self.ui.logtextBrowser.append("Successfully compare the coverage of test cases before and after optimization......")
        self.ui.logtextBrowser.append("*" * 50)
...
class ResultTest:
    def __init__(self):
        ...
        self.ui = QUiLoader().load('testresult.ui')
        result_title = 'Serial No.' + "\t" + 'seed test dataset' + "\t" + "\t" + "\t" + 'generate test dataset' + "\t" + "\t" + "\t" + 'label'
        self.ui.genTextBrowser.append(result_title)
