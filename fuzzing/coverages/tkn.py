# -*- coding: utf-8 -*-
import numpy as np
from fuzzing.coverages.utils import get_layer_outs, get_layer_outs_new, percent_str
...
def measure_tkn_with_pattern(model, test_inputs, k, skip=None, outs=None):
    ...
    print(layer_count)
    for input_index in range(len(test_inputs)):  # out_for_input是输入层的输出
        pattern = []

        for layer_index in range(layer_count):  # layer_out 是神经网络每层的所有输出
...
class DeepGaugeLayerLevelCoverage:
    ...
    def test(self, test_inputs):
        ...
        for input_index in range(len(test_inputs)):  # out_for_input输入层的输出
            pattern = []

            for layer_index in range(layer_count):  # layer_out 层对所有输入的输出
