...
import numpy as np
import sys
# from cleverhans.future.tf2.attacks.basic_iterative_method import basic_iterative_metho
from cleverhans.attacks import SaliencyMapMethod, FastGradientMethod, CarliniWagnerL2, BasicIterativeMethod
# from cleverhans.tf2.attacks
from cleverhans.utils_keras import KerasModelWrapper
# from cleverhans.utils import KerasModekWrapper
...
def get_layer_outs_old(model, class_specific_test_set):
    inp = model.input  # 输入占位符
    outputs = [layer.output for layer in model.layers]  # 所有层输出
    functors = [K.function([inp] + [K.learning_phase()], [out]) for out in outputs]  # 计算函数
    # Testing
...
def get_layer_outs(model, test_input, skip=[]):
    ...
    functors = [K.function([inp], [out]) for out in outputs]  # 计算函数
...
def calc_major_func_regions(model, train_inputs, skip=None):
    ...
    for layer_index, layer_out in enumerate(outs):  # layer_out 是所有层的输出
...
def get_layer_outputs_by_layer_name(model, test_input, skip=None):
    ...
    outputs = {layer.name: layer.output for index, layer in enumerate(model.layers)
               if (index not in skip and 'input' not in layer.name)}  # 每层的输出 (除了函数的输入)
    functors = {name: K.function([inp], [out]) for name, out in outputs.items()}  # 计算函数
