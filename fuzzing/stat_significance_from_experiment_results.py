...
mcts_deephunter_t = (stat.mean(mcts_results) - stat.mean(deephunter_results)) / np.sqrt(
    stat.variance(mcts_results) / (len(mcts_results) - 1) + stat.variance(deephunter_results) / (
                len(deephunter_results) - 1))
...
mcts_tensorfuzz_t = (stat.mean(mcts_results) - stat.mean(tensorfuzz_results)) / np.sqrt(
    stat.variance(mcts_results) / (len(mcts_results) - 1) + stat.variance(tensorfuzz_results) / (
                len(tensorfuzz_results) - 1))
